...
def generate_launch_description():
    ...
    pkg_ros_ign_sim = FindPackageShare(package='ros_ign_gazebo').find('ros_ign_gazebo')
    ...
    start_gazebo_server_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ros_ign_sim, 'launch', 'ign_gazebo.launch.py')),
        launch_arguments=[('ign_args', [' -r -s -v 4 ', world_path])])

    # Start Gazebo client (GUI) if not headless
    start_gazebo_client_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ros_ign_sim, 'launch', 'ign_gazebo.launch.py')),
        launch_arguments={'ign_args': ['-g ']}.items(),
